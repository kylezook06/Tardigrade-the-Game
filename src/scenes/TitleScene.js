...
class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    let headerBottomY = (h / 2) - 205;

    if (this.textures.exists("ui_logo")) {
      const logo = this.add.image(w / 2, h / 2 - 215, "ui_logo");
      const maxW = 300;
      const maxH = 90;
      ...
      const title = this.add.text(w / 2, h / 2 - 215, "Tardigrade: The Game", {
        fontFamily: "Arial",
        fontSize: "46px",
        color: "#ffffff"
      }).setOrigin(0.5);
    ...
    this.add.text(w / 2, headerBottomY + 6, "Choose a mode", {
      fontFamily: "Arial",
      fontSize: "18px",
      color: "#d7e6ff",
      alpha: 0.9
